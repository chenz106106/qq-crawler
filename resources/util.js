...
function ptuiV(A) {
	if (A != pt.version) {
		pt.cleanCache(pt.getJsSrc());
		pt.reportAttr2(220827, 0.05)
	}
};

function getPvId(){
	var d = Math.round(Math.random() * 2147483647) * (new Date).getUTCMilliseconds() % 1E10;
...
function getSsId(){
	var f = Math.round(Math.random() * 2147483647) * (new Date).getUTCMilliseconds() % 1E10;
